...
from sales.serializers import OrderSerializer, OrderListSerializer
...
def getAllOrder(request):
	...
	order = pagination.paginate_data(order)
...
def searchOrder(request):
	order = OrderFilter(request.GET, queryset=Order.objects.all())
	order = order.qs

	print('searched_products: ', order)
	...
	if len(order) > 0:
		...
		return Response({'detail': f"There are no order matching your search"}, status=status.HTTP_400_BAD_REQUEST)
